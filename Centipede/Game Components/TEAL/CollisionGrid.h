...
private:
	...
	void GetCollidingCandidate( sf::FloatRect &r, Candidate &cndtes, int &n)
	{
		// Early exit
		...
		Cell *cell;
		Candidate::iterator it = cndtes.begin();
		n=0;
			...
			for( b = cellminy; b <= cellmaxy; b++)
			{
				...
				for( unsigned int k=0 ; k <  cell->count ; k++) 
				{
					*it = cell->vector[k]; 
					...
					if (n == (int) cndtes.size()) cndtes.resize(cndtes.size() + HashgridSize);
					it++;
				}
			}
		}
